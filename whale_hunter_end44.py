...
CONFIG = {
    ...
    # اعتبارسنجی
    "validation_times": [1, 2, 4],  # دقیقه
    "validation_weights": [20, 30, 50],  # درصد
...
class MarketAPI:
    ...
    def fetch(source=None):
        ...
        if data:
            # ذخیره در تاریخچه و OHLCV
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            for item in data:
                price_history.add(item['symbol'], item['price'], item['volume'])
                c.execute('''INSERT INTO ohlcv (symbol, open, high, low, close, volume)
                             VALUES (?, ?, ?, ?, ?, ?)''',
                          (item['symbol'], item['price'], item['high_24h'], 
                           item['low_24h'], item['price'], item['volume']))
            conn.commit()
            conn.close()
    ...
    def calculate_score(validations):
        ...
        for i, v in enumerate(validations):
            if v and v.get('is_valid'):
                score += weights[i] if i < len(weights) else 0
    ...
    def check_pending_signals(market_data):
        ...
        for signal_id, signal in list(SignalValidator.pending_signals.items()):
            ...
            for i, minutes in enumerate(validation_times):
                if elapsed >= minutes and signal['validations'][i] is None:
            ...
            if elapsed >= pump_time:
                ...
                if is_valid and score >= CONFIG['min_score_for_trade']:
                    ...
                    c.execute('''INSERT INTO signals 
                                 (symbol, signal_type, entry_price, final_status, score, source)
                                 VALUES (?, ?, ?, 'valid', ?, 'pump_dump')''',
                              (symbol, signal_type, current_price, score))
            ...
            if market_data:
                ...
                if AutoTrader.is_running:
                    ...
                    if can:
                        ...
                        if queue:
                            AutoTrader.execute_trade(queue[0])
...
def api_market():
    ...
    return jsonify({'success': data is not None, 'data': data or [], 'source': source})
...
def api_pump_dumps():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT * FROM pump_dumps ORDER BY timestamp DESC LIMIT 50')
    columns = [desc[0] for desc in c.description]
...
def api_validation_data():
    """دریافت داده‌های اعتبارسنجی (سیگنال، نهنگ، پامپ)"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    
    # دریافت سیگنال‌های معتبر و در انتظار
    ...
    # دریافت نهنگ‌ها
    c.execute('SELECT * FROM whales ORDER BY timestamp DESC LIMIT 50')
...
def api_config():
    ...
    try:
        ...
        allowed_keys = ['min_score_for_trade', 'trade_amount', 'stop_loss', 'take_profit']
        
        updated = False
        for key in allowed_keys:
            if key in data:
                # تبدیل نوع داده در صورت نیاز
                if key == 'min_score_for_trade':
    ...
    <script>
        ...
        async function fetchData() {
            try {
                ...
                // Whales
                const whalesRes = await fetch('/api/whales');
                const whales = await whalesRes.json();
                renderWhales(whales);
                
                // Signals
                const signalsRes = await fetch('/api/signals?status=' + signalFilter);
                const signals = await signalsRes.json();
                renderSignals(signals);
                
                // Pumps
                const pumpsRes = await fetch('/api/pump_dumps');
                const pumps = await pumpsRes.json();
                renderPumps(pumps);
                
                // Update stats
                document.getElementById('statWhales').textContent = whales.whales.length;
                document.getElementById('statValid').textContent = signals.stats.valid;
                document.getElementById('statPending').textContent = signals.stats.pending;
                document.getElementById('statAccuracy').textContent = signals.stats.accuracy + '%';
                document.getElementById('statPumpDump').textContent = pumps.length;
                document.getElementById('statFlow').textContent = '$' + formatNumber(whales.flow.net);
        ...
        function renderMarket() {
            ...
            let data = marketData.filter(item => {
